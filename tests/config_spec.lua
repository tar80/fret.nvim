...
describe('Fret.config', function()
  describe('.set_options()', function()
    ...
    it('mapkeys field is required in the option table', function()
      ...
      opts.altkeys = { lshift = 'JKLUIOPNMHY', rshift = 'FDSAREWQVCXZGTB' }
    ...
    describe('max_length', function()
      it('field must be integer', function()
        opts.fret_max_length = '100'
        assert.has_error(function()
          config.set_options(opts)
        end)
      end)
      it('default is 1000', function()
        config.set_options(opts)
        assert.is.same(1000, vim.g.fret_max_length)
      end)
      it('minimum is 500', function()
        opts.fret_max_length = -1
        config.set_options(opts)
        assert.is.same(500, vim.g.fret_max_length)
      end)
    end)
